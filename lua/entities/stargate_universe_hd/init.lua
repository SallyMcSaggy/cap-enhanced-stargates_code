...
ENT.Sounds = {
	Open=Sound("junko_enoshima/universe_hd/gate_open.mp3"),
	Travel=Sound("stargate/gate_travel.mp3"),
	Close=Sound("junko_enoshima/universe_hd/gate_close_hd.mp3"),
	...
	Chevron=Sound("junko_enoshima/universe_hd/chevlock_hd.mp3"),
	Fail=Sound("junko_enoshima/universe_hd/fail.mp3"),
	Activate=Sound("kripalida/sgu_hd/begin_roll.wav"),
	GateRoll=Sound("stargate/universe/long_gate_roll.wav"),
	StopRoll=Sound("junko_enoshima/universe_hd/ring_stop_hd.mp3"),
}

ENT.Mats = {
  Off="kripalida/sg_hd/universechevronoff.vmt",
  On="kripalida/sg_hd/universechevronon.vmt",
}

ENT.MatsSymb = {
  Off="kripalida/sg_hd/symbols.vmt",
  On="kripalida/sg_hd/symbolson.vmt",
...
function ENT:BeforeCloak(entity,cloacking_device)
	if (entity:GetClass()=="stargate_universe_hd") then
...
function ENT:SpawnFunction(p,t)
	...
	local e = ents.Create("stargate_universe_hd");
	e:SetPos(pos);
	e:DrawShadow(false);
    e:Spawn();
	e:Activate();
...
function ENT:SpinSound(spin)
    if(spin)then
	    ...
	    self.RollSound:SetSoundLevel(100);
...
function ENT:RingTickUniverse()
	--for _,self in pairs(ents.FindByClass("stargate_universe_hd")) do
...
function ENT:ChevronSound(chev)
    ...
    self.Entity:EmitSound(self.Sounds.Chevron,100,math.random(100,97));
end
...
function ENT:TriggerInput(k,v,mobile,mdhd,ignore)
	...
	elseif(k == "Activate Chevrons" and not self.NewActive and not self.WireManualDial)then
		if (v >= 1 and self:CheckEnergy(true,true)) then
	    	...
	    	self.Chevron:SetMaterial("kripalida/sg_hd/universechevronon");
	    	self.Entity:SetNWBool("ActChevronsL",true);
	    else
			self.Chevron:SetMaterial("kripalida/sg_hd/universechevronoff");
...
function ENT.Sequence:DialFail(instant_stop,play_sound,fail)
	...
	if(self.Entity.Active or play_sound) then
		action:Add({f=self.EmitSound,v={self.Entity,self.Sounds.Fail,90,math.random(100,105)},d=0});-- Fail sound
	end
...
if (StarGate and StarGate.CAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "stargate_universe_hd", StarGate.CAP_GmodDuplicator, "Data" )
end