...
-- Models
ENT.Models = {
	Base="models/kripalida/sg_hd/base_sg1_hd.mdl",
	Ring="models/kripalida/sg_hd/ring_sg1_hd.mdl",
	Chevron="models/kripalida/sg_hd/chevron_hd.mdl",
}
-- Sounds
ENT.Sounds = {
	Open=Sound("kripalida/sg1_hd/sg1_new.mp3"),
	...
	Chevron={Sound("junko_enoshima/sg1_hd/chevron_hd.mp3"),Sound("junko_enoshima/sg1_hd/chevron_hd.mp3")},
...
ENT.SGCChevron = {
	Sound("junko_enoshima/sg1_hd/sgc_chev_13_hd.mp3"),
	Sound("junko_enoshima/sg1_hd/sgc_chev_13_hd.mp3")
}
...
function ENT:SpawnFunction(p,t)
	...
	local e = ents.Create("stargate_sg1_hd");
	...
	elseif(k == "Ring Speed Mode" and IsValid(self.Ring) and not self.Active and (not self.NewActive or self.WireManualDial)) then
		...
		elseif (v >= 3) then
			...
			if (not self.NewActive or self.WireManualDial) then
				self.Ring.Entity:SetKeyValue("maxspeed",180);
				if (self.Ring.WireMoving) then self.Ring:Fire("start","",0); end
				if (self.RingSound) then self.RingSound:ChangePitch(130,0); end
			end
		elseif (v <= -1) then
			self.RingSpeed = -1;
			if (not self.NewActive or self.WireManualDial) then
...
if (StarGate and StarGate.CAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "stargate_sg1_hd", StarGate.CAP_GmodDuplicator, "Data" )
end